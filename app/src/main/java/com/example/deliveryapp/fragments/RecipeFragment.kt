...
class RecipeFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recipeViewModel =
            ViewModelProvider(requireActivity()).get(RecipeFragmentViewModel::class.java)
        ...
        return ComposeView(requireContext()).apply {
            ...
            setContent {
                ...
                recipeViewModel?.detailReceipt?.value?.title?.let { title ->
                    ...
                    val time: String = when (val readyTime =
                        recipeViewModel?.detailReceipt!!.value?.readyInMinutes) {
                        1 -> "$readyTime minute"
                        else -> "$readyTime minutes"
                    }
                    val healthScore =
                        "Score: ${recipeViewModel?.detailReceipt!!.value?.healthScore}"
